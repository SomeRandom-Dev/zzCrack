...
class main:
    def __init__(self):
        self.passFound_print = (
            lambda tries, word: f"{Style.RESET_ALL}Password was found after {Fore.LIGHTCYAN_EX + str(tries)} {Style.RESET_ALL}tries. The password is: {Fore.LIGHTGREEN_EX + word}"
        )
        ...
        for word in wordlist:
            try:
                if self.done:
                    ...
                    if confirm == "y":
                        self.save_state(
                            archive_dir, wordlist_inp, word, stream, wordlist
                        )
                        return
                    self.done = False
                ...
                tries += 1
                if stream:
                    print(self.tries_print(tries, word))
    ...
    def wlist_crack_entry(self, archive_dir, wordlist_inp, showoutput_b, resume_index):
        ...
        try:
            try:
                _archive = ZipFile(archive_dir, "r")
            ...
            self.wlist_crack(
                tries, wordlist, _archive, showoutput_b, archive_dir, wordlist_inp
            )
        except IOError:
    ...
    def generate_charlist(self, charset):
        ...
        charlist = "".join(
            "".join([chars[int(c)] for c in charset.split("+") if c in allowed_chars])
        )
        if charlist == "":
    ...
    def memsafe_generate_bruteforce_list(self, charset, max_lenght):
        ...
        return (
            ...
            for candidate in chain.from_iterable(
                product(charlist, repeat=i) for i in range(1, max_lenght + 1)
            )
        )
    ...
    def bruteforce_crack_entry(
        self, archive_dir, charset, max_letters, showoutput_b, resume_index
    ):
        tries = 0
        try:
            try:
                ...
                sys.exit(1)
            print("Started...")
            self.bruteforce_crack(
                tries, None, _archive, showoutput_b, charset, max_letters
            )
        except IOError:
    ...
    ######################
    def save_state(
        self, archive_dir, wordlist_inp, current_word, showoutput_b, generated_list
    ):
        while True:
            saveState = input(
                "\nDo you want to save the Current State and continue from here later? [--restore]  (y/n) > "
            ).lower()
            if saveState == "y":
                try:
                    ...
                    f = open(name, "w")
                    f.write(
                        f"{os.path.abspath(archive_dir)},{os.path.abspath(wordlist_inp)},{int(showoutput_b)},{generated_list.index(current_word)}"
                    )
                    f.close()
    ...
    def restore(self):
        try:
            savefiles = [
                f
                for f in os.listdir("./stateSaves")
                if os.path.isfile(os.path.join("./stateSaves", f))
            ]
            ...
            self.wlist_crack_entry(
                resfile[0], resfile[1], bool(int(resfile[2])), int(resfile[3])
            )
        except FileNotFoundError:
            print("No restore files found")
        except IndexError as ex:
            print(
                f"Restore file index given is not valid or the restorefile is broken {ex}"
            )
        except Exception as ex:
            print(f"Unexpected error: {ex}")
    ...
    def _noargs(self):
        ...
        if attack_type == "1":
            ...
            if showOutPut.lower() == "y":
                ...
                showoutput_b = False
            self.bruteforce_crack_entry(
                archive_dir, charset, max_letters, showoutput_b, None
            )
        else:
            print("Invalid Input")
        sys.exit(1)
    ...
    def _args(self):
        if (
            ...
            and options.reslist == False
            and options.restore == None
        ):
        ...
        if options.wordlist != None:
            ...
            print(
                "\n".join(
                    f"{savefiles.index(saves)} > {savefiles[savefiles.index(saves)]}"
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option("-f", dest="archive", metavar=" ", help="Archive To Crack")
    parser.add_option(
        "-w",
        dest="wordlist",
        help="Enter the name Or directory to the wordlist for a wordlist attack",
        metavar=" ",
    )
    parser.add_option(
        "-b",
    ...
    parser.add_option(
        "-c",
        dest="charlist",
        help="(Required for Bruteforce)\nWhich characters to use in the Bruteforce attack (combine to use multiple, separated by +) 0: Only small letters | 1: Only big letters | 2: Only numbers | 3: Only signs | 4: Everything",
        metavar=" ",
    )  # byt
    parser.add_option(
        "-l",
        dest="maxlenght",
        help="The max lenght for the bruteforce passwords (Required for Bruteforce)",
        metavar=" ",
    )
    parser.add_option(
        "-s",
    ...
    parser.add_option(
        ...
        "--restore",
        dest="restore",
        help="Restore state From save (do --rl for a list of vaild options)",
        metavar=" ",
    )
    parser.add_option(
        "--rl",
